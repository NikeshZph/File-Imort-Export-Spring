...
@Query(value = "select * from employee where lower(address) like lower(concat('%', :searchtext, '%')) or lower(department) like lower(concat('%', :searchtext, '%')) or lower(name) like lower(concat('%', :searchtext, '%')) ", nativeQuery = true)
List<Employee> findEmployeeBySearch(String searchtext);

    // Add a method to handle empty search text
